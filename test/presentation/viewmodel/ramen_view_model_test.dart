import 'package:flutter_test/flutter_test.dart';
import 'package:mockito/annotations.dart';
import 'package:mockito/mockito.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:noodle_timer/core/di/app_providers.dart';
import 'package:noodle_timer/data/dto/ramen_data.dart';
import 'package:noodle_timer/domain/repository/ramen_repository.dart';
import 'package:noodle_timer/domain/repository/user_repository.dart';
import 'ramen_view_model_test.mocks.dart';

@GenerateMocks([RamenRepository, UserRepository])
void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  final ramenDataJson = {
    'ramenData': [
      {
        'brandId': 1,
        'brandName': '농심',
        'ramens': [
          {
            'ramenIndex': 100,
            'ramenName': '신라면',
            'ramenImage': 'https://i.namu.wiki/i/...',
            'ramenSpicy': '매운맛',
            'ramenDescription': '한국을 대표하는 매운 라면',
            'ramenRecipe': '1. 물 550ml에...',
            'afterSeasoning': false,
            'cookTime': 270,
          },
          {
            'ramenIndex': 101,
            'ramenName': '안성탕면',
            'ramenImage': 'https://i.namu.wiki/i/...',
            'ramenSpicy': '순한맛',
            'ramenDescription': '담백한 맛의 전통적인 라면',
            'ramenRecipe': '1. 물 550ml에...',
            'afterSeasoning': false,
            'cookTime': 270,
          },
        ],
      },
    ],
  };

  final testBrands = RamenData.fromJson(ramenDataJson).toEntity().brands;
  final testRamenList = testBrands.expand((brand) => brand.ramens).toList();

  group('RamenViewModel 브랜드 관리 테스트', () {
    late MockRamenRepository mockRamenRepository;
    late MockUserRepository mockUserRepository;
    late ProviderContainer container;

    setUp(() {
      mockRamenRepository = MockRamenRepository();
      mockUserRepository = MockUserRepository();
      container = ProviderContainer(
        overrides: [
          ramenRepositoryProvider.overrideWithValue(mockRamenRepository),
          userRepositoryProvider.overrideWithValue(mockUserRepository),
        ],
      );
    });

    tearDown(() => container.dispose());

    test('브랜드 로드 시 "나의 라면 기록"이 가장 먼저 추가되어야 한다', () async {
      // given
      when(
        mockRamenRepository.loadBrands(),
      ).thenAnswer((_) async => testBrands);
      when(
        mockRamenRepository.loadAllRamen(),
      ).thenAnswer((_) async => testRamenList);
      when(mockUserRepository.getCurrentUserId()).thenReturn('test-user-id');
      when(
        mockUserRepository.getCookHistories('test-user-id'),
      ).thenAnswer((_) async => []);

      final viewModel = container.read(ramenViewModelProvider.notifier);

      // when
      await viewModel.loadBrands();

      // then
      final state = container.read(ramenViewModelProvider);
      expect(state.brands.length, 2);
      expect(state.brands[0].name, '나의 라면 기록');
    });

    test('브랜드 선택 시 해당 라면 목록이 currentRamenList에 업데이트되어야 한다', () async {
      // given
      when(
        mockRamenRepository.loadBrands(),
      ).thenAnswer((_) async => testBrands);
      when(
        mockRamenRepository.loadAllRamen(),
      ).thenAnswer((_) async => testRamenList);
      when(mockUserRepository.getCurrentUserId()).thenReturn('test-user-id');
      when(
        mockUserRepository.getCookHistories('test-user-id'),
      ).thenAnswer((_) async => []);

      final viewModel = container.read(ramenViewModelProvider.notifier);
      await viewModel.loadBrands();

      // when
      viewModel.selectBrand(1);

      // then
      final state = container.read(ramenViewModelProvider);
      expect(state.currentRamenList, isNotEmpty);
      expect(state.currentRamenList.length, 2);
      expect(state.currentRamenList[0].name, '신라면');
      expect(state.currentRamenList[1].name, '안성탕면');
    });
  });

  group('RamenViewModel 검색 기능 테스트', () {
    late MockRamenRepository mockRamenRepository;
    late MockUserRepository mockUserRepository;
    late ProviderContainer container;

    setUp(() {
      mockRamenRepository = MockRamenRepository();
      mockUserRepository = MockUserRepository();
      container = ProviderContainer(
        overrides: [
          ramenRepositoryProvider.overrideWithValue(mockRamenRepository),
          userRepositoryProvider.overrideWithValue(mockUserRepository),
        ],
      );
    });

    tearDown(() => container.dispose());

    test('초기 상태에서 allRamen이 로드되어야 한다', () async {
      // given
      when(
        mockRamenRepository.loadAllRamen(),
      ).thenAnswer((_) async => testRamenList);

      // when
      await container.read(ramenViewModelProvider.notifier).loadRamens();

      // then
      final state = container.read(ramenViewModelProvider);
      expect(state.allRamen, isNotEmpty);
      expect(state.allRamen.length, 2);
    });

    test('검색어가 없으면 searchResults가 비어있어야 한다', () async {
      // given
      when(
        mockRamenRepository.loadAllRamen(),
      ).thenAnswer((_) async => testRamenList);

      final viewModel = container.read(ramenViewModelProvider.notifier);
      await viewModel.loadRamens();

      // when
      viewModel.updateSearchKeyword('');

      // then
      final state = container.read(ramenViewModelProvider);
      expect(state.searchKeyword, isEmpty);
      expect(state.searchResults, isEmpty);
      expect(state.isSearching, false);
    });

    test('검색어가 주어졌을 때 해당 라면만 필터링되어야 한다', () async {
      // given
      when(
        mockRamenRepository.loadAllRamen(),
      ).thenAnswer((_) async => testRamenList);

      final viewModel = container.read(ramenViewModelProvider.notifier);
      await viewModel.loadRamens();

      // when
      viewModel.updateSearchKeyword('신라면');

      // then
      final state = container.read(ramenViewModelProvider);
      expect(state.searchKeyword, '신라면');
      expect(state.searchResults.length, 1);
      expect(state.searchResults.first.name, '신라면');
      expect(state.isSearching, true);
      expect(state.hasSearchResults, true);
    });

    test('부분 검색어로도 라면을 찾을 수 있어야 한다', () async {
      // given
      when(
        mockRamenRepository.loadAllRamen(),
      ).thenAnswer((_) async => testRamenList);

      final viewModel = container.read(ramenViewModelProvider.notifier);
      await viewModel.loadRamens();

      // when
      viewModel.updateSearchKeyword('면');

      // then
      final state = container.read(ramenViewModelProvider);
      expect(state.searchResults.length, 2);
      expect(
        state.searchResults.any((ramen) => ramen.name.contains('면')),
        true,
      );
    });

    test('검색어를 지우면 검색 결과가 초기화되어야 한다', () async {
      // given
      when(
        mockRamenRepository.loadAllRamen(),
      ).thenAnswer((_) async => testRamenList);

      final viewModel = container.read(ramenViewModelProvider.notifier);
      await viewModel.loadRamens();
      viewModel.updateSearchKeyword('신라면');

      // when
      viewModel.updateSearchKeyword('');

      // then
      final state = container.read(ramenViewModelProvider);
      expect(state.searchKeyword, isEmpty);
      expect(state.searchResults, isEmpty);
      expect(state.isSearching, false);
    });
  });
}